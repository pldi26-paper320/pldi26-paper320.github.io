...
(class Main {
  ...
  static {
    ...
    (class tabs {
      static {
        Main.tabs = tabs;
        let tmp17;
        tmp17 = runtime.safeCall(Main.#query(".overlay"));
        this.overlay = tmp17;
        (class typer {
          static {
            tabs.typer = typer;
            let tmp18, tmp19, tmp20;
            tmp18 = runtime.safeCall(Main.#query("#tab-typer"));
            this.el = tmp18;
            tmp19 = runtime.safeCall(typer.el.querySelector("div.results"));
            this.results = tmp19;
            tmp20 = runtime.safeCall(typer.el.querySelector("#show-traces"));
            this.showTraces = tmp20;
            const diagnostics$class = class diagnostics {
              constructor() {
                let tmp21, tmp22, tmp23;
                tmp21 = runtime.safeCall(typer.el.querySelector(".diagnostics"));
                this.elem = tmp21;
    ...
    lambda3 = (undefined, function (event) {
      let tmp17;
      tmp17 = Main.tabs.overlay.setAttribute("data-loading", "true");
      return globalThis.setTimeout(Main.run, 1)
    });
  ...
  static showDiagnostics(elems, diagnostics1, problematicStages) {
    let scrut, tmp, tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp7;
    if (globalThis.Array.isArray(diagnostics1) && diagnostics1.length === 0) {
      ...
      tmp7 = runtime.safeCall(diagnostics1.join("\n"));
  ...
  static run() {
    ...
    lambda = (undefined, function () {
      let startTime, blocks, res, elapsedSeconds, problematicStages1, scrut, scrut1, scrut2, scrut3, scrut4, allDiagnostics, tmp2, tmp3, tmp4, tmp5, tmp6, tmp7, tmp8, tmp9, tmp10, tmp11, tmp12, tmp13, tmp14, tmp15, tmp16, tmp17, tmp18, tmp19, tmp20, tmp21, tmp22, tmp23, tmp24, tmp25, lambda2, lambda3;
      tmp2 = runtime.safeCall(globalThis.Date.now());
      startTime = tmp2;
      tmp3 = Main.splitTestBlocks(Main.#editor.value);
      blocks = tmp3;
      tmp4 = runtime.safeCall(globalThis.console.log(blocks));
      tmp5 = runtime.safeCall(MLscript.compile(blocks));
      res = tmp5;
      tmp6 = runtime.safeCall(globalThis.Date.now());
      tmp7 = tmp6 - startTime;
      tmp8 = tmp7 / 1000;
      tmp9 = runtime.safeCall(tmp8.toPrecision(3));
      elapsedSeconds = tmp9;
      tmp10 = "Compilation took " + elapsedSeconds;
      tmp11 = tmp10 + " seconds.";
      tmp12 = runtime.safeCall(globalThis.console.log(tmp11));
      tmp13 = runtime.safeCall(globalThis.console.log(res));
      problematicStages1 = [];
      scrut = res.lexer.diagnostics.length > 0;
      if (scrut === true) {
        tmp14 = runtime.safeCall(problematicStages1.push("lexer"));
      } else {
        tmp14 = runtime.Unit;
      }
      scrut1 = res.parser.diagnostics.length > 0;
      if (scrut1 === true) {
        tmp15 = runtime.safeCall(problematicStages1.push("parser"));
      } else {
        tmp15 = runtime.Unit;
      }
      scrut2 = res.elaborator.diagnostics.length > 0;
      if (scrut2 === true) {
        tmp16 = runtime.safeCall(problematicStages1.push("elaborator"));
      } else {
        tmp16 = runtime.Unit;
      }
      scrut3 = res.resolver.diagnostics.length > 0;
      if (scrut3 === true) {
        tmp17 = runtime.safeCall(problematicStages1.push("resolver"));
      } else {
        tmp17 = runtime.Unit;
      }
      scrut4 = res.typer.diagnostics.length > 0;
      if (scrut4 === true) {
        tmp18 = runtime.safeCall(problematicStages1.push("typer"));
      } else {
        tmp18 = runtime.Unit;
      }
      lambda2 = (undefined, function (result) {
        let tmp26, tmp27, tmp28, tmp29, tmp30, tmp31;
        tmp26 = "<div class=\"source\"><pre><code>" + result.source;
        tmp27 = tmp26 + "</code></pre></div>\n";
        tmp28 = tmp27 + "<div class=\"output\">";
        tmp29 = tmp28 + result.result;
        tmp30 = tmp29 + "</div>\n";
        tmp31 = tmp30 + "</div>";
        return "<div class=\"typer-result\">\n" + tmp31
      });
      tmp19 = lambda2;
      tmp20 = Iter.mapping(res.typer.results, tmp19);
      tmp21 = Iter.joined(tmp20, "");
      Main.tabs.typer.results.innerHTML = tmp21;
      allDiagnostics = [
      ...
      tmp22 = Main.showDiagnostics(Main.tabs.typer.diagnostics, allDiagnostics, problematicStages1);
      tmp23 = runtime.safeCall(Main.tabs.overlay.removeAttribute("data-loading"));
      tmp24 = runtime.safeCall(globalThis.document.querySelectorAll(".latexty"));
      lambda3 = (undefined, function (x) {
        let left, right, display;
        left = "$$";
        right = "$$";
        display = false;
        return globalThis.renderMathInElement(x, {
        "delimiters": [
            {
      ...
      tmp25 = lambda3;
      return Iter.each(tmp24, tmp25)
    });
    tmp = lambda;
    lambda1 = (undefined, function (error) {
      let errorDisplay, tmp2, tmp3, tmp4, tmp5;
      tmp2 = runtime.safeCall(globalThis.console.log(error));
      tmp3 = runtime.safeCall(globalThis.document.createElement("error-display"));
      errorDisplay = tmp3;
      tmp4 = runtime.safeCall(errorDisplay.setError(error));
      tmp5 = runtime.safeCall(Main.#outputPanel.appendChild(errorDisplay));
      return runtime.safeCall(Main.tabs.overlay.removeAttribute("data-loading"))
    });
    tmp1 = lambda1;
    return Runtime.try_catch(tmp, tmp1)
  } 
